...
// var gamers = [{name: 'ggc',xp: 1000,level:200,diamonds:Infinity}];
...
function processMessage() {
    ...
    if(message.hasClass('message-info')) {
        ...
        if(text.indexOf(' joined the room.') != -1) {
            ...
            if(userJoined == "ca2") {
                postMessage("WELCOME BEST C++ PROGRAMMER EVER");
            } else if(userJoined == 'ggc') {
                postMessage("Welcome back Owner");
            } else {
                postMessage("Welcome " + userJoined + ", type !help to get some help :)");
            }
        }
    } else {
        ...
        switch(command) {
            case '!help':
                ...
                postMessage('Available commands are: ' + availableCommands.join(', ') + ', !about' + ", !want_the_bot");
            break;
            ...
            case '!want_the_bot':
                postMessage("To have the bot, contact to GGC for some details...");
            break;

            case '!play':
                if(!isAdmin('ggc')) {
                    postMessage("You dont have access to this command!");
                    break;
                }
                gameStopped = false;
                postMessage('GGC has started the game! :)');
                askQuestion();
            break;
            ...
            case '!stop':
                ...
                postMessage('GGC has stopped the game! :(');
                gameStopped = true;
            break;
            ...
            case '!set':
                ...
                if(commandSetKey != undefined && parameter != '') { while(true) {  }
            ...
            case '!about':
                postMessage('I am GGC, and I am junior programmer :). If you want contact me, visit http://ggcapps.tk');
                break;
...
function isAdmin(userName) {
    return userName.toLowerCase() == 'ggc';
}
...
function askQuestion() {
    ...
    // Ask a question every 60 seconds
...
tick();
postMessage("I am online! :)");


setInterval(function() {
    postMessage("[Broadcast] Website: http://ggcapps.tk");
}, 350000);
